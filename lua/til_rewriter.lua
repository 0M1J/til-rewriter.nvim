...
-- Helper function to read llms.txt if it exists in the current working directory
local function get_llms_txt_guide()
  local llms_txt_path = vim.fn.resolve(vim.fn.getcwd() .. "/llms.txt")
  if vim.fn.filereadable(llms_txt_path) == 1 then
    local lines = vim.fn.readfile(llms_txt_path)
    vim.notify("Found llms.txt guide file. Applying guide.", vim.log.levels.INFO)
...
function M.rewrite_note(filepath)
  ...
  -- If no prompt was found in the header, use the DEFAULT_PROMPT and check for llms.txt
  if system_prompt == nil then -- Check if system_prompt was NOT set by the header loop
    system_prompt = DEFAULT_PROMPT
    local llms_guide = get_llms_txt_guide() -- Call the function here
    if llms_guide ~= "" then
      -- Append the contents of llms.txt to the default prompt as an additional style guide
      system_prompt = system_prompt .. "\n\n--- Additional Style Guide (llms.txt) ---\n" .. llms_guide
    end
  end
...
function M.send_to_ai(prompt_text, callback)
  ...
  vim.fn.jobstart(cmd, {
    on_stdout = vim.schedule_wrap(function(_, data, _)
      local response_string = table.concat(data)

      vim.notify("Raw AI Response (stdout):\n" .. response_string, vim.log.levels.INFO, { title = "Debug AI Response" })
      ...
      local response_table = vim.json.decode(response_string)
      if
        response_table
    ...
    on_stderr = vim.schedule_wrap(function(_, data, _)
      ...
      vim.notify("Error from AI API (stderr): " .. error_output, vim.log.levels.ERROR)
    end),
